...
while ( $rank = $db->fetchArray($result) ) {
    ...
    if ($rank['rank_special']==0){
        ...
        $ranks[$i]['image'] = ($rank['rank_image'] > '')?'<img src="'.ICMS_URL.'/uploads/'.$rank['rank_image'].'" alt="" />':'&nbsp;';
    } else {
        $sranks[$i]['title'] = $rank['rank_title'];
        $sranks[$i]['image'] = ($rank['rank_image'] > '')?'<img src="'.ICMS_URL.'/uploads/'.$rank['rank_image'].'" alt="" />':'&nbsp;';
    }
}
$icmsTpl->assign('ranks', $ranks);
$icmsTpl->assign('sranks', $sranks);
